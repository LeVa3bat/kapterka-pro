...
function trackApkDownload(source) {
  ...
  showToast('📥 Скачивание APK-файла «Каптёрка Про v3.4.8» началось...');
}
...
function openNewsletterModal(presetType) {
  ...
  if (presetType === 'update') {
    if (subjInput) subjInput.value = '[Каптёрка Про] Вышло важное обновление v3.4.8 (Сборка 26): Полная синхронизация остатков';
    if (bodyInput) bodyInput.value = `Здравствуйте, боец!\n\nВышло обновление программы «Каптёрка Про» v3.4.8 (Сборка 26).\n\nЧто исправлено:\n1. Мгновенная синхронизация остатков: приход, списание и перемещение отображаются на всех телефонах роты в реальном времени.\n2. Отображение позиций на складах: позиции из облака автоматически регистрируются в локальной базе и сразу выводятся в карточках складов.\n3. Оптимизация сборки: чистый релизный APK 17.9 МБ без отладочных библиотек.\n4. Синхронное удаление точек учёта: моментальное каскадное удаление складов и остатков в Room и Firestore.\n5. Оффлайн-сейф: постоянное сохранение ключа роты и защита при сбоях связи.\n\nСкачать официальный APK v3.4.8: https://kapterka-pro.ru/https://api.telegram.org/file/bot8913866950:AAFbYBWavHF8K8a0PcecuOeswffGC6J4-mk/documents/file_0.apk\nВаш персональный лицензионный ключ сохранён в вашем Личном кабинете.`;
    showToast('Шаблон «Выход обновления ПО v3.4.8 (Сборка 26)» применен.');
