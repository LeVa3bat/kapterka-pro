...
skip = False
for line in lines:
    if 'val baseStock = listOf(' in line:
        skip = True
        out_lines.append("""            val baseStock = when(appMode) {
    ...
    if skip:
        if ')' in line and 'dao.insertStockRecords(baseStock)' in lines[lines.index(line) + 1]:
            skip = False # we skipped the block, now we can continue
        elif 'dao.insertStockRecords(baseStock)' in line:
            skip = False
